...
export default function Topbar() {
  ...
  return (
    <header
      className="fixed top-0 right-0 bg-surface border-b border-border z-30 flex justify-between items-center px-8 gap-6"
      style={{ left: 'var(--sidebar-width)', height: 'var(--topbar-height)' }}
    >
      {/* ── Search ───────────────────────────────────────── */}
      <div className="flex-1">
        <div className="flex items-center gap-2.5 px-4 h-9 rounded-full border border-border bg-app text-secondary text-sm cursor-pointer hover:border-brand/40 transition-colors duration-150 max-w-sm">
          <Search size={14} strokeWidth={2} />
          <span className="flex-1 select-none">Search transactions…</span>
          <kbd className="text-xs bg-surface border border-border rounded-md px-1.5 py-0.5 font-mono leading-none">
            ⌘K
          </kbd>
        </div>
      </div>
                  ...
                  state.notifications.map(n => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-start gap-3 px-5 py-4 cursor-pointer transition-colors duration-100 ${
                          n.read ? 'hover:bg-surface-hover' : 'bg-brand/[0.03] hover:bg-brand/[0.06]'
                        }`}
                      >
          ...
          >
            <span
              className={`w-2 h-2 rounded-full flex-shrink-0 ${
                state.role === 'admin' ? 'bg-brand' : 'bg-success'
              }`}
            />
